...
const templateListLink = (list) => {
  return list.reduce((accumulator, actual) => {
    return (accumulator += `
      <li>
      <a class="link" href="${actual.link}" target="_blank">${actual.name}</a>
      </li>
     `);
  }, "");
};
...
const createCards = (infos) => {
  const template = infos.reduce((accumulator, actual) => {
    ...
    <div class="card__body">
      ...
      <p>
        ${
          actual.practicalExample
            ? `<span class="strong">Illustration:</span> ${actual.practicalExample} `
            : ""
        }
      </p>
      <p>
      ${
        actual.tools
          ? `<p><strong>Tool Suggestion:</strong></p> 
             <ul class="tools__list">
             ${templateListLink(actual.tools)}
             </ul>
            `
          : ""
      }
